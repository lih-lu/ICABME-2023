...
# opt = tf.keras.optimizers.Adam(0.0001 * hvd.size()) # this approach is sensitive to our model 
...
opt = hvd.DistributedOptimizer(opt, backward_passes_per_step=1, average_aggregated_gradients='True', op=hvd.Average,  compression=compression)
...
###                                optimizer=tf.keras.optimizers.Adam(learning_rate=init_lr), metrics=[dice_coefficient])
                               optimizer=opt, metrics=[dice_coefficient])
        else:
            unet_model.compile(loss='categorical_crossentropy', 
...
###        unet_model.compile(loss='mse', optimizer=tf.keras.optimizers.Adam(learning_rate=init_lr))
        unet_model.compile(loss='mse', experimental_run_tf_function=False, optimizer=opt)
    else:
        raise ValueError('mode must be either `classification` or `regression`')
